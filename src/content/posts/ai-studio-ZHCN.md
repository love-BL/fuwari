...
(() => {
  ...
  const EXACT = new Map([
    ...
    ['Model selection', '模型选择'], ['All', '全部'], ['Featured', '精选'], ['Images', '图像'], ['New', '新'],['Adjust harmful response settings', '调整有害内容设置'],['Run safety settings', '运行安全设置'],['Rerun', '重试'],['Delete', '删除'],
...
// ==UserScript==
// @name         Google AI Studio 汉化脚本（Tampermonkey 版）
// @namespace    http://tampermonkey.net/
// @version      1.2.0
// @description  将 Google AI Studio (aistudio.google.com) 页面的主要 UI 元素翻译为中文（Tampermonkey）
// @author       Betsy
// @match        https://aistudio.google.com/*
// @grant        none
// @run-at       document-start
   ...
   const EXACT = new Map([
    ...
    function translateText(text) {
        ...
        if (EXACT.has(trimmed)) {
            return original.replace(trimmed, EXACT.get(trimmed));
        }
        // 模糊匹配
        for (const { pattern, replace } of PARTIAL) {
            if (pattern.test(original)) {
                return original.replace(pattern, replace);
            }
        }
        // 动态匹配
        for (const { pattern, replace } of DYNAMIC) {
            if (pattern.test(original)) {
                return original.replace(pattern, (...args) => {
                    const last = args[args.length - 1];
                    const groups = last && typeof last === 'object' ? last : undefined;
    ...
    function translateTextNode(node) {
        ...
        if (nv && nv !== node.nodeValue) {
            ...
            if (node.parentElement) {
                node.parentElement.setAttribute(MARK, '1');
            }
        }
    }
            ...
            for (const m of mutations) {
                if (m.type === 'childList') {
                    for (const n of m.addedNodes) {
                        if (n.nodeType === Node.ELEMENT_NODE) {
                            walk(n);
                            if (n.matches && n.matches(DIALOG_SELECTORS)) {
                                walk(n);
                            }
                            if (n.querySelector) {
                                n.querySelectorAll(DIALOG_SELECTORS).forEach(d => walk(d));
                            }
                            if (n.tagName === 'IFRAME') {
                                handleIframe(n);
                            }
                    ...
                    translateAttributes(m.target);
                    if (m.target.matches && m.target.matches(DIALOG_SELECTORS)) {
                        walk(m.target);
                    }
                }
            }
